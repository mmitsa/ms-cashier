...
public record InvoiceDto(
    long Id, string InvoiceNumber, InvoiceType InvoiceType, DateTime InvoiceDate,
    int? ContactId, string? ContactName, int WarehouseId, string WarehouseName,
    PriceType PriceType, decimal SubTotal, decimal DiscountAmount, decimal TaxAmount,
    decimal TotalAmount, decimal PaidAmount, decimal DueAmount,
    PaymentMethod PaymentMethod, PaymentStatus PaymentStatus, string? Notes,
...
public record InvoiceSearchRequest(
    DateTime? DateFrom, DateTime? DateTo, int? ContactId,
    PaymentMethod? PaymentMethod, PaymentStatus? PaymentStatus,
    InvoiceType? InvoiceType, int Page = 1, int PageSize = 50);
