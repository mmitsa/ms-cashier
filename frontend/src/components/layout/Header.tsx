import { Bell, LogOut, ChevronDown, ShoppingCart, LayoutDashboard } from 'lucide-react';
  ...
  return (
    <header className="h-14 bg-white border-b border-gray-100 flex items-center justify-between px-6 shrink-0">
      ...
      <div className="flex items-center gap-3">
        <h2 className="font-bold text-gray-900 text-base">{currentModule?.label || 'MS Cashier'}</h2>
        <span className="text-xs text-gray-400 hidden sm:inline">الكونسول الإداري</span>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        {isInPOS && (
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 bg-gray-100 text-gray-700 rounded-xl text-xs font-bold hover:bg-gray-200 transition"
          >
        ...
        <div className="relative">
          ...
          >
            ...
            <div className="hidden md:block text-right">
              <p className="text-xs font-medium text-gray-900 leading-tight">{user?.fullName || 'المدير'}</p>
              <p className="text-[10px] text-gray-400">{ROLE_LABELS[user?.role || ''] || user?.role || 'مستخدم'}</p>
            </div>
          ...
          {showMenu && (
            <>
              ...
              <div className="absolute left-0 top-full mt-2 w-56 bg-white rounded-xl shadow-xl border border-gray-100 z-50 py-2">
                <div className="px-4 py-3 border-b border-gray-100">
                  <p className="text-sm font-semibold text-gray-900">{user?.fullName}</p>
                  <p className="text-xs text-gray-500">@{user?.username}</p>
                  <p className="text-xs text-brand-600 mt-0.5 font-medium">{ROLE_LABELS[user?.role || ''] || user?.role}</p>
                </div>
                <button
