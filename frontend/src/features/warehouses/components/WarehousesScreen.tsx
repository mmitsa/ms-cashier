    ...
    <div className="space-y-6" dir="rtl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">إدارة المخازن ونقل البضاعة</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="card p-12 text-center">
          ...
          <p className="text-gray-500 font-medium">لا توجد مخازن بعد</p>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-4 mt-3">
            <div className="flex items-center gap-1">
              ...
              <span className="text-sm text-gray-600">{warehouse.totalItems} صنف</span>
            </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between p-5 border-b border-gray-100">
          <h2 className="text-lg font-bold text-gray-900">إنشاء مخزن جديد</h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            />
            <span className="text-sm font-medium text-gray-700">مخزن رئيسي</span>
          </label>
...
function TransferStockModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between p-5 border-b border-gray-100">
          <h2 className="text-lg font-bold text-gray-900">نقل بضاعة بين المخازن</h2>
          <button onClick={onClose} className="p-2 rounded-lg hover:bg-gray-100 transition-colors">
            <X size={20} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          {/* Product Search */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">إضافة أصناف</label>
            <div className="relative">
              ...
              <input
                ...
                className="w-full pr-10 pl-4 py-2.5 rounded-xl border border-gray-200 focus:border-brand-500 outline-none text-sm"
                placeholder="ابحث عن صنف..."
              />
            </div>
            {productSearch && products.length > 0 && (
              <div className="mt-1 border border-gray-200 rounded-xl max-h-40 overflow-y-auto bg-white shadow-lg">
                {products.map((p) => (
                  <button
                    key={p.id}
                    type="button"
                    ...
                    className="w-full text-right px-4 py-2.5 text-sm hover:bg-gray-50 transition-colors border-b border-gray-50 last:border-0 disabled:opacity-40"
                  >
          ...
          {items.length > 0 && (
            <div className="space-y-2">
              <p className="text-sm font-medium text-gray-700">الأصناف المختارة:</p>
              {items.map((item) => (
                <div key={item.productId} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                  ...
                  <input
                    ...
                    className="w-20 px-3 py-1.5 rounded-lg border border-gray-200 text-center text-sm focus:border-brand-500 outline-none"
                  />
                  <button
                    type="button"
